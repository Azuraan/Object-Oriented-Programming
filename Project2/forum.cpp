...
void forum::print_sorted(){
	int i=0;
	cout<<"Sorted printing of forum "<<title<<endl;
	Tree *temp=NULL,*t = new Tree(*threads[i]);
	for(i=1;threads[i]!=NULL;i++){
		temp = new Tree(*threads[i]);
		t->TreeEnhance(temp);
	}
	t->printTree();
	cout<<"Finished printing; deleting Tree..."<<endl;
	delete t;
}
...
:id(p.id),title(p.title),creator(p.creator),cdate(p.cdate),text(p.text){}
...
post* thread::get_post_ptr(int i){
			return posts[i];
}